...
import inputs
import master_code
from master_code import ChemDASH
import symmetry
...
def main(args=None):
    ...
    parser = argparse.ArgumentParser(
        description="ChemDASH Structure Prediction Code",
        epilog="This code is developed by Paul Sharp at the University of Liverpool. "
        "Please direct enquiries to Paul.Sharp@liverpool.ac.uk",
    ...
    parser.add_argument(
        ...
        nargs="?",
        default=None,
        help="The seed name for this calculation,"
        ' which forms the name of the ".input" and ".atoms" files.',
    )
    parser.add_argument(
        "-i",
        "--input",
    ...
    # include: type="File --see docs"
    parser.add_argument(
        "-s",
        "--symm",
        "--symmetry",
        metavar="<cif file>",
        help="Use spglib to look for higher symmetry in the supplied cif file, "
        'and write to a new file "<cif file>_symm.cif".',
    )
    # include: type="File --see docs"
    ...
    parser.add_argument(
        ...
        metavar="<input file>",
        help="Write an input file that includes all keywords with their default values "
        "to the given file and exit.",
    )  # include: type="File --see docs"
    ...
    if opts.parse is not None:
        ...
        for input_file in opts.parse:
            ...
            if len(errors) > 0:
                ...
                print(
                    'Please refer to the error log "{0}".\n'.format(
                        calc_name + ".error"
                    )
                )
                inputs.report_input_file_errors(
                    errors, input_file, calc_name + ".error"
                )
            else:
                print('The file "{0}" is a valid input file.'.format(input_file))
                print()
                print(
                    "\tPlease note that keywords, options and settings for GULP/VASP "
                    "will be verified in GULP/VASP calculations."
                )
                print(
                    "\tAlso, when the initial structure is composed, "
                    "the validity of any supplied swap groups will be verified at that point."
                )
                print()
    ...
    if opts.symm is not None:
        ...
        if not os.path.isfile(opts.symm):
            ...
            print()
            print(
                'The structure generated by spglib has been written to "{0}".'.format(
                    symmetrised_structure_file
                )
            )
            print()
    ...
    if opts.seed is not None:
        ...
        ChemDASH(calc_seed).run_chemdash()
    # else:
    #     print('Please supply the calculation seed from your ".input" file -- USAGE:
    #     "python prime <seed>" for input file "<seed>.input".')
