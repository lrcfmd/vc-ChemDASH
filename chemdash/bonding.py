...
R0_values = {
    ...
    "Al3--F-1": 1.545,
    "Al3--O-2": 1.651,
    "Al2--O-2": 1.789,  # not a real value remember to delete after testing
    "As3--O-2": 1.789,
    "As3--S-2": 2.272,
    "As5--F-1": 1.620,
    "As5--O-2": 1.767,
    ...
    "Co2--O-2": 1.692,
    "Co3--C2": 1.634,
    "Cr2--O-2": 1.761,  # new value from O. C. Gagné and F. C. Hawthorne,
    # Acta Crystallogr B Struct Sci Cryst Eng Mater., 2015, 71(5), 562–578
    "Cr3--F-1": 1.657,
    "Cr3--O-2": 1.724,
    "Cr6--O-2": 1.794,
    "Cs1--Cl-1": 2.791,
    ...
    "Mn2--O-2": 1.790,
    "Mn2--Br-1": 2.34,
    "Mn3--Br-1": 2.34,  # taken from "Mn2--Br-1"
    "Mn3--O-2": 1.760,
    "Mn3--Cl-1": 2.14,
    "Mn4--O-2": 1.753,
    "Mo6--O-2": 1.907,
...
def bond_valence_sum_for_atoms(structure, neighbour_dist=5.0, B=0.37):
    ...
    atom_indices = [int(atom.index) for atom in structure if atom.symbol != "X"]
    ...
    for i in atom_indices:
        neighbour_indices, distances = get_distances_to_neighbours(
            structure, i, neighbour_dist
        )
        bond_valence_sums[i] = compute_bond_valence_sum(
            structure, i, neighbour_indices, distances, B
        )
...
# =============================================================================
def bond_valence_sum_for_sites(
    structure, dopant_atoms=None, dopant_atom_charges=None, neighbour_dist=5.0, B=0.37
):
    """
    ...
    del reduced_structure[
        [atom.index for atom in reduced_structure if atom.symbol == "X"]
    ]
    ...
    if dopant_atoms:
        for dopant, dopant_charge in zip(dopant_atoms, dopant_atom_charges):
            charges[dopant.symbol] = float(dopant_charge)

        atom_symbols += list(set([x.symbol for x in dopant_atoms]))
    ...
    for i in range(0, len(structure)):
        ...
        for atom in atom_symbols:
            ...
            bond_valence_sum[atom_key] = compute_bond_valence_sum(
                structure, i, neighbour_indices, distances, B
            )
    ...
    for index, bond_atom in enumerate(bonded_atoms):
        ...
        if structure[bvs_atom].charge > structure[bond_atom].charge:
            bond_string = (
                bvs_atom_string
                + "--"
                + structure[bond_atom].symbol
                + str(int(structure[bond_atom].charge))
            )
        else:
            bond_string = (
                structure[bond_atom].symbol
...
def get_distances_to_neighbours(structure, bvs_atom, neighbour_dist=5.0):
    ...
    nl = ase.neighborlist.NeighborList(
        [neighbour_dist] * len(structure), self_interaction=False, bothways=True
    )
    nl.update(structure)
    full_indices, full_offsets = nl.get_neighbors(bvs_atom)

    # Remove vacancies from list of neighbours
    neighbour_indices, offsets = list(
        zip(*[(i, o) for i, o in zip(full_indices, full_offsets) if symbols[i] != "X"])
    )
...
def check_R0_values(structure):
    ...
    cation_symbols = [
        atom.symbol + str(int(atom.charge)) for atom in structure if atom.charge > 0.0
    ]
    anion_symbols = [
        atom.symbol + str(int(atom.charge)) for atom in structure if atom.charge < 0.0
    ]
