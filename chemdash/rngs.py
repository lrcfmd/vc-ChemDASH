...
class NR_Ran(object):
    ...
    def __init__(self, seed):
        ...
        assert (
            seed != self.v
        ), "ERROR in rng.NR_Ran.__init__ -- seed must not equal value of intrinsic parameter, v={0:d}".format(
            v
        )
...
def generate_random_seed(seed_bits):
    ...
    random_seed_command = (
        "od -vAn -N" + seed_bytes + " -tu" + seed_bytes + " < /dev/urandom"
    )
...
if __name__ == "__main__":
    ...
    print(
        "The mean of the random numbers is {0:f} +/- {1:f}".format(
            np.mean(ran_nums), np.sqrt(var / float(2 * N))
        )
    )
    print(
        "The variance of the random numbers is {0:f} +/- {1:f}".format(
            var, var * np.sqrt(2.0 / (float(2 * N) - 1.0))
        )
    )
