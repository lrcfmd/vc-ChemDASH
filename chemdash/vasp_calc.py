...
def multi_stage_vasp_calc(
    ...
    assert len(additional_settings) >= num_calcs, (
        "ERROR in vasp_calc.multi_stage_vasp_calc() -- number of VASP "
        "calculations is set to {0:d}, but only {1:d} sets of additional "
        "settings are provided.".format(num_calcs, len(additional_settings))
    )
    ...
    with open(vasp_file, mode="w") as vasp_out:
        ...
        for i in range(0, num_calcs - 1):
            ...
            structure.atoms, structure.energy, result = run_vasp(
                structure.atoms, vasp_settings
            )
            ...
            # if there is no OUTCAR, assume something has gone wrong with calculation
            if not os.path.isfile("OUTCAR"):
                print(
                    "No OUTCAR found in directory. Check your vasp input parameters as this "
                    "error may be raised when one of these is incorrect."
                )
                result = "unconverged"
                break

            converged = converged_in_one_scf_cycle("OUTCAR")
        ...
        if continue_to_final_stage:
            ...
            # With calculation completed up to the penultimate stage,
            # run final stage of calculation until convergence within a single SCF cycle is achieved
            vasp_settings = main_settings.copy()
            ...
            for i in range(0, max_convergence_calcs):
                ...
                if converged_in_one_scf_cycle("OUTCAR", n_ionic_steps):
                    result = "converged"
    ...
    files_to_remove = ["CHGCAR", "WAVECAR", "XDATCAR"]
    ...
    try:
        os.rename("vasprun.xml", "vasprun_" + str(structure.index) + ".xml")
...
def run_vasp(structure, vasp_settings):
    ...
    # Use ASE calculator -- the use of **kwargs in the function call allows
    # us to set the desired arguments using a dictionary
...
# =============================================================================
def set_vasp_script(
    vasp_script, executable, num_cores, vasp_pseudopotentials, method="mpirun"
):
    """
    ...
    with open(vasp_script, mode="w") as script:
        ...
        if method == "srun":
            script.write(f"import os\nexitcode = os.system('{method} {executable}')")
        elif method == "mpirun":
            if num_cores > 1:
                script.write(
                    f"import os\nexitcode = os.system('{method} -np {num_cores} "
                    f"{executable}')"
                )
            ...
            print(
                "THIS METHOD IS NOT VALID. ONLY 'srun' and 'mpirun' are valid options!"
            )
...
# =============================================================================
def converged_in_one_scf_cycle(outcar_file, n_steps=1):
    """
