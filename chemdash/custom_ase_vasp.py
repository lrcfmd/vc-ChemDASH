...
def atomtypes_outpot(posfname, numsyms):
    ...
    # of POSCAR/CONTCAR.
    fnames = [posfname.replace("POSCAR", "POTCAR").replace("CONTCAR", "POTCAR")]
    fnames.append(posfname.replace("POSCAR", "OUTCAR").replace("CONTCAR", "OUTCAR"))
    ...
    raise IOError("Could not determine chemical symbols. Tried files " + str(tried))
...
def _read_outcar_frame(lines, header_data):
    ...
    for n, line in enumerate(lines):
        ...
        if "direct lattice vectors" in line:
            ...
            mag_x = [float(cl(lines[n + i + nskip]).split()[-1]) for i in range(natoms)]
        ...
        #              for i in range(natoms)]
            ...
            for i in range(nbands):
                ...
                f_n[i] = float(parts[2])
            kpts.append(SinglePointKPoint(w, spinc, ikpt, eps_n=eps_n, f_n=f_n))
...
def read_vasp_xdatcar(filename="XDATCAR", index=-1):
    ...
    while True:
        ...
        coords = [np.array(fd.readline().split(), np.float) for ii in range(total)]
...
def write_vasp(
    ...
    if np.any(atoms.get_cell_lengths_and_angles() == 0.0):
        raise RuntimeError(
