...
try:
    from chemdash.dope import random_atom_dope
except ModuleNotFoundError:
    from dope import random_atom_dope
...
def determine_maximum_swaps(structure, selection):
    ...
    reduced_structure = reduce_structure(structure.atoms.copy(), selection)
    ...
    if selection == "atoms-vacancies":
        elements_list = [
            "A" if element != "X" else element for element in elements_list
        ]
    ...
    try:
        ...
        print(
            "Failure in gulp_calc.determine_maximum_swaps() when attempting to swap {0}".format(
                selection
            )
        )
        max_swaps = 0
        ...
        remaining_elements = [
            element for element in elements_list if element != modal_element
        ]
...
def choose_number_of_atoms_to_swap(max_swaps, weightings_method, rng, pinned_weighting):
    ...
    if weightings_method == "arithmetic":
        ...
        sys.exit(
            "ERROR in swap.choose_number_of_atoms_to_swap() -- {0} is not a valid choice for constructing the number weightings.".format(
                weightings_method
            )
        )
...
def generate_linearly_decreasing_weightings(num_elements):
    ...
    assert (
        num_elements > 0
    ), "ERROR in swap.generate_linearly_decreasing_weightings() -- number of elements is {0:d}, but should be greater than 0.".format(
        num_elements
    )
...
def generate_geometrically_decreasing_weightings(num_elements):
    ...
    Generate a set of probabilities for a sorted list of elements such that for
    all elements being considered, each is less likely to be chosen than that
    which precedes it. The probabilities will cover the full set of elements,
    ...
    assert (
        num_elements > 0
    ), "ERROR in swap.generate_geometrically_decreasing_weightings() -- number of elements is {0:d}, but should be greater than 0.".format(
        num_elements
    )
...
def generate_uniform_weightings(num_elements):
    ...
    assert (
        num_elements > 0
    ), "ERROR in swap.generate_uniform_weightings() -- number of elements is {0:d}, but should be greater than 0.".format(
        num_elements
    )
...
def pair_pinned_weightings(num_elements, pinned_weighting):
    ...
    assert (
        num_elements > 0
    ), "ERROR in swap.pair_pinned_weightings() -- number of elements is {0:d}, but should be greater than 0.".format(
        num_elements
    )
...
def swap_atoms(
    ...
    swap_list: list,
    ranking_dict: object,
    directed_num_atoms: object,
    predefined_vacancies: object,
    vacancy_exclusion_radius: object,
    rng: object,
    force_vacancy_swaps: object,
    mag_moms: object,
    magnetic_structure_only: object = False,
) -> (Atoms, str):
    """
    ...
    Parameters
    ----------
    magnetic_structure_only : boolean
        Determines if only magnetic moments are to be swapped
    mag_moms: list of integers
        List of magnetic moments for the current structure under consideration
    ...
    # occurrence of a particular element later.
    ...
    # get original magnetic moments and positions for mapping the swap back for magnetic moments
    if mag_moms is not None:
        current_positions = copy.deepcopy(structure.atoms.positions.tolist())
        current_mag_moments = copy.deepcopy(mag_moms)
    else:
        current_mag_moments = []
    print(structure.atoms)
    if all(species in ranking_dict for species in swap_list):
        print("Directed Swapping is being used")
        swap_indices, atom_indices, swap_list, new_order = select_atoms_directed(
            swap_list,
            new_order,
            structure.atoms,
            ranking_dict,
        ...
        swap_indices, swap_list, new_order = select_atoms_at_random(
            swap_list,
            new_order,
            structure.atoms,
            predefined_vacancies,
    ...
    swap_positions = get_swap_positions(structure.atoms, swap_indices)
    # Write swap text before applying swap
    swap_text = []
    scaled_pos = structure.atoms.get_scaled_positions()
    ...
    swap_positions = reorder_positions(
        swap_positions, np.asarray(new_order), swapped_elements
    )

    # only swap atoms if not looking at swapping magnetic moments
    if not magnetic_structure_only:
        # Set new positions in structure
        for i in range(0, len(swap_list)):
            # i have added the .atoms here when the class was implemented but not sure why it
            # actually broke in the first instance...
            structure.atoms[swap_indices[i]].position = swap_positions[i]

    # Reset magnetic moments to be in same site position as before
    if not all(x == 0 for x in mag_moms):
        new_positions = structure.atoms.positions.copy().tolist()

        for i in range(0, len(swap_list)):
            new_positions[swap_indices[i]] = list(swap_positions[i])

        # generates the new indices to swap magnetic moments with
        new_indices = [
            current_positions.index(i)
            for i, j in zip(new_positions, current_positions)
            if i in current_positions
        ]
        print(len(current_mag_moments))
        print(len(new_indices))
        new_mag_moments = [current_mag_moments[x] for x in new_indices]
        structure.atoms.set_initial_magnetic_moments(new_mag_moments)

    for atom in structure.atoms:
        if all(math.isnan(x) for x in atom.position):
            print(f"The atom {atom} has no position")
            structure.dopant_atoms.append(atom.symbol)
            structure.dopant_atom_charges.append(atom.charge)
    ...
    return structure, swap_text, new_mag_moments
...
def generate_selection_pool(elements_list, num_swaps):
    ...
    num_swaps : integer
        Number of atoms/vacancies we will swap -- such that a non-trivial swap is possible.
    ...
    assert (
        num_swaps > 1
    ), "ERROR in swap.generate_selection_pool() -- num_swaps should be greater than 1, but is: {0:d}".format(
        num_swaps
    )
...
def generate_swap_list(selection_pool, num_swaps, rng):
    ...
    assert num_swaps <= len(
        selection_pool
    ), "ERROR in swap.generate_swap_list() -- num_swaps should not be greater than the length of the selection_pool, but we are trying to swap {0:d} atoms from: {1}".format(
        num_swaps, selection_pool
    )
...
# =============================================================================
def select_atoms_at_random(
    swap_list, new_order, atoms, predefined_vacancies, vacancy_exclusion_radius, rng
):
    """
    ...
    new_order : string
        Rearranged list of atoms that we will swap, including the species of atoms.
    ...
    species_dict["A"] = [atom.index for atom in atoms if atom.symbol != "X"]

    for species in species_list:
        # if atom not (yet) in structure
        if not any([atom.symbol == species for atom in atoms]):
            print(f"No {species} atoms in structure")
        else:
            species_dict[species] = [
                atom.index for atom in atoms if atom.symbol == species
            ]
    ...
    for swap_list_index in atom_indices:
        ...
        atom_index = species_dict[atom_to_swap].pop(
            rng.int_range(0, remaining_atoms_of_this_species)
        )
    ...
    for swap_list_index in vacancy_indices:
        ...
        if not predefined_vacancies:
            ...
            vacancies_to_remove = find_vacancies_near_a_selected_vacancy(
                atoms, vacancy_index, species_dict["X"], vacancy_exclusion_radius
            )
            ...
            if len(vacancies_to_remove) <= len(species_dict["X"]) - vacancies_to_swap:
                ...
                print(
                    "WARNING -- we have tried to remove {0:d} vacancies, but there are only {1:d} vacancies on the grid (excluding those we need to swap)".format(
                        len(vacancies_to_remove),
                        len(species_dict["X"]) - vacancies_to_swap,
                    )
                )
...
def select_atoms_directed(
    ...
    all_species = atoms.get_chemical_symbols().copy()
    num_swaps = len(swap_list)
    ...
    for species in species_list:
        ...
        num_atoms_dict[species] = min(
            num_to_swap + directed_num_atoms, num_in_structure
        )

    # Choose atoms from the rankings, using the minima hopping method
    for swap_list_index in atom_indices:
        atom_to_swap = swap_list[swap_list_index]
        atom_index = ranking_dict[atom_to_swap].pop(
            rng.int_range(-num_atoms_dict[atom_to_swap], 0)
        )
    ...
    for swap_list_index in vacancy_indices:
        ...
        if not predefined_vacancies:
            ...
            if (
                len(vacancies_to_remove)
                <= len(ranking_dict["X"][swap_string]) - vacancies_to_swap
            ):
    ...
    return swap_indices, atom_indices, swap_list, new_order


# =============================================================================
def find_vacancies_near_a_selected_vacancy(
    structure, selected_vacancy_index, vacancy_indices, vacancy_exclusion_radius
):
    """
    ...
    if len(vacancy_indices) > 0:
        atom_vacancy_distances = structure.get_distances(
            selected_vacancy_index, vacancy_indices, mic=True
        )
...
# =============================================================================
def get_swap_positions(atoms, swap_indices):
    """
    ...
    swap_positions = [[math.nan, math.nan, math.nan]] * num_swaps
    ...
    for i in range(num_swaps):
        try:
            ...
            continue
            # swap_positions[i] = np.array([-i, -i, -i])
...
def permute_atoms(old_order, rng, force_vacancy_swaps):
    ...
    element_order = [
        element
        for elements, count in element_counter.most_common()
    ...
    for element in element_order:
        ...
        available_sites = [
            ...
            if new_order[index] == "" and old_order[index] != element
        ]

        num_element = element_counter[element]

        if len(available_sites) < num_element or (
            element == "X" and not force_vacancy_swaps
        ):
            available_sites = [
                index for index in range(len(new_order)) if new_order[index] == ""
            ]
    ...
    if any([new_order[i] == old_order[i] for i in range(num_atoms)]):
        ...
        for atom in unswapped_atoms:
            sites_to_swap_to = [
                ...
                if new_order[index] != new_order[atom]
                and old_order[index] != new_order[atom]
            ]

            swap_atom = sites_to_swap_to[rng.int_range(0, len(sites_to_swap_to))]

            new_order[atom], new_order[swap_atom] = (
                new_order[swap_atom],
                new_order[atom],
            )
...
# =============================================================================
def accept_swap(energy_difference, temperature, rng):
    """
    ...
    # Evaluate the Boltzmann factor for the current structures
...
def reduce_structure(structure, atom_filter):
    ...
    if atom_filter == "cations" or atom_filter == "magnetic-moments":
        ...
        del structure[
            [atom.index for atom in structure if atom.symbol not in atom_list]
        ]
...
# =============================================================================
def check_previous_structures(new_atoms, positions, atomic_numbers, pos_tol=1.0e-12):
    """
    ...
    del new_atoms[[atom.index for atom in new_atoms if atom.symbol == "X"]]
    ...
    sort_indices = np.lexsort(new_atoms.get_positions().T)

    new_positions = new_atoms.get_positions()[sort_indices]
    new_atomic_numbers = new_atoms.get_atomic_numbers()[sort_indices]
...
def initialise_default_swap_groups(structure, swap_groups):
    ...
    for group in swap_groups[:]:
        ...
        reduced_structure = reduce_structure(structure.atoms.copy(), group[0])
...
def verify_swap_groups(structure, swap_groups):
    ...
    for non-trivial swaps of the specific group of atoms. If not, a list of
    ...
    for i, group in enumerate(swap_group_names):
        ...
        reduced_structure = reduce_structure(structure.atoms.copy(), group)
        elements_list = reduced_structure.get_chemical_symbols() + [
            x.symbol for x in structure.dopant_atoms
        ]
        ...
        if num_elements >= 2:
            ...
            swap_group_errors.append(
                '"{0}" has been specified as a swap group, but there are insufficient different species '
                "to enable non-trivial swaps to be made -- there are {1:d} species of {2}.".format(
                    group, num_elements, group
                )
            )
...
def check_elements_in_custom_swap_groups(swap_groups, all_atoms):
    ...
    for group in swap_group_names:
        ...
        if not set(swap_group_elements).issubset(all_atoms):
            swap_group_errors.append(
                '"{0}" has been specified as a swap group, '
                "but there are no {1} atoms in the structure.".format(
                    group,
                    ", ".join(
                        [
                            str(x)
                            for x in [
                                y for y in swap_group_elements if y not in all_atoms
                            ]
                        ]
                    ),
                )
...
def update_atom_rankings(structure, ranking_measure):
    ...
    -------
    structure : ChemDASH Structure
        The structure, including rankings for atoms according to their BVS
    ...
    unique_elements = list(
        set(
            structure.atoms.get_chemical_symbols()
            + [a.symbol for a in structure.dopant_atoms]
        )
    )
    try:
...
def rank_bvs(structure, unique_elements):
    ...
    bvs_rank = [
        abs(abs(charges[index]) - structure.bvs_atoms[index]) for index in atom_indices
    ]
    structure.ranked_atoms["A"] = rank_indices(bvs_rank, atom_indices)

    for element in sorted(unique_elements):
        element_indices = [
            atom.index for atom in structure.atoms if atom.symbol == element
        ]
        bvs_rank = [
            abs(abs(charges[index]) - structure.bvs_atoms[index])
            for index in element_indices
        ]
        structure.ranked_atoms[element] = rank_indices(bvs_rank, element_indices)
    for i, element in enumerate(structure.dopant_atoms):
        structure.ranked_atoms[element.symbol].append(i + len(structure.atoms))
    ...
    if len(vacancy_indices) > 0:
        ...
        for atom_key in structure.bvs_sites[vacancy_indices[0]]:
            ...
            if all(
                [atom_key in structure.bvs_sites[index] for index in vacancy_indices]
            ):
                charge = float(atom_key.split("/")[1])
                # The factor of -1 is here because we want the best vacancies for the appropriate element.
                bvs_rank = [
                    (-1.0 * abs(abs(charge) - structure.bvs_sites[index][atom_key]))
                    for index in vacancy_indices
                ]
                structure.ranked_atoms["X"][
                    "X--" + atom_key.split("/")[0]
                ] = rank_indices(bvs_rank, vacancy_indices)
...
def rank_site_potential(structure, unique_elements):
    ...
    for element in unique_elements:
        ...
        pot_rank = [
            structure.potentials[index] / charges[index] for index in element_indices
        ]
    ...
    if len(vacancy_indices) > 0:
        ...
        for atom in atom_symbols:
            ...
            charge_list = [
                reduced_structure[i].charge
                for i in range(len(reduced_structure))
                if reduced_structure[i].symbol == atom
            ]
            charge = max(charge_list, key=charge_list.count)

            # The factor of -1 is here because we want the best vacancies for the appropriate element.
            pot_rank = [
                -1.0 * structure.potentials[index] / charge for index in vacancy_indices
            ]
            structure.ranked_atoms["X"]["X--" + atom] = rank_indices(
                pot_rank, vacancy_indices
            )
...
def rank_bvs_plus(structure, unique_elements):
    ...
    vacancy_cation_indices = [
        index for index in vacancy_indices if structure.potentials[index] < 0.0
    ]
    vacancy_anion_indices = [
        index for index in vacancy_indices if structure.potentials[index] > 0.0
    ]
        ...
        for atom_key in structure.bvs_sites[vacancy_indices[0]]:
            ...
            if all(
                ...
                if charge > 0.0:
                    ...
                    bvs_rank = [
                        (-1.0 * abs(abs(charge) - structure.bvs_sites[index][atom_key]))
                        for index in vacancy_anion_indices
                    ]
                    structure.ranked_atoms["X"][
                        "X--" + atom_key.split("/")[0]
                    ] = rank_indices(bvs_rank, vacancy_anion_indices)

                    bvs_rank = [
                        (-1.0 * abs(abs(charge) - structure.bvs_sites[index][atom_key]))
                        for index in vacancy_cation_indices
                    ]
                    structure.ranked_atoms["X"][
                        "X--" + atom_key.split("/")[0]
                    ] += rank_indices(bvs_rank, vacancy_cation_indices)
...
def find_desired_atoms(bvs_sites, atom_indices):
    ...
    for atom in atom_indices:
        ...
        charges = [
            float(bvs_values[i][0].split("/")[1]) for i in range(len(bvs_values))
        ]

        # Find how far from the ideal each value is, and rank according to this.
        bvs_rank = [
            (abs(abs(charges[i]) - bvs_values[i][1])) for i in range(len(bvs_values))
        ]
...
def rank_indices(rankings, indices):
    ...
    assert len(rankings) == len(
        indices
    ), "ERROR in swap.rank_indices() -- there are {0:d} ranking values, and {1:d} indices. They should be the same".format(
        len(rankings), len(indices)
    )
