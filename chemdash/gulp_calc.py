...
def multi_stage_gulp_calc(
    ...
    assert (
        len(gulp_files) >= num_calcs
    ), "ERROR in gulp_calc.multi_stage_gulp_calc() -- number of GULP calculations is set to {0:d}, but only {1:d} filenames are provided.".format(
        num_calcs, len(gulp_files)
    )
    assert (
        len(additional_keywords) >= num_calcs
    ), "ERROR in gulp_calc.multi_stage_gulp_calc() -- number of GULP calculations is set to {0:d}, but only {1:d} sets of additional keywords are provided.".format(
        num_calcs, len(additional_keywords)
    )
    assert (
        len(additional_options) >= num_calcs
    ), "ERROR in gulp_calc.multi_stage_gulp_calc() -- number of GULP calculations is set to {0:d}, but only {1:d} sets of additional options are provided.".format(
    ...
    for i in range(0, num_calcs):
        ...
        if os.path.isfile(calc_files["output"]):
            ...
            if any(
                pot_keyword in gulp_keywords for pot_keyword in ["pot", "potential"]
            ):
...
def run_gulp(
    ...
    elif read_outcome(calc_files["output"]) == timeout_outcome or check_timed_out(
        calc_files["output"]
    ):
        result = "timed out"
    ...
    if os.path.isfile(gulp_res_file):
        ...
        for i, line in enumerate(res_file):
            ...
            if "totalenergy" in line:
                ...
                try:
                    energy = float(line.split()[1]) / float(
                        len(strip_vacancies(structure.copy()))
                    )
                except ValueError:
                    energy = line.split()[1]
        ...
        try:
            ...
            print(
                'WARNING in "gulp_calc.update_atoms_from_restart_file()" -- no valid coordinate type found in ".res" file'
            )
        else:
...
# =============================================================================
def create_input_file_from_restart_file(
    gulp_input_file,
    gulp_restart_file,
    unit_cell,
    gulp_keywords,
...
def execute_gulp_command_or_script(gulp_job_name, gulp_output):
    ...
    if "ASE_GULP_COMMAND" in os.environ:
        ...
        gulp_command = os.environ.get("ASE_GULP_COMMAND").replace(
            "PREFIX", gulp_job_name
        )
        ...
        raise RuntimeError(
            "Please set either GULP_COMMAND or GULP_SCRIPT environment variable"
        )
