...
def write_custom_cif(
    fd, images, cif_format=None, wrap=True, labels=None, loop_keys=None, moments=None
) -> None:
    ...
    magnetic: bool
        Defines if a system is magnetic allowing for the writing of mcif files.

    moments: list
        Defines the magnetic moments to be written to mcif file. If not specified,
        will use get_magnetic_moments() or get_initial_magnetic_moments() to get an array
        of moments.
    """
    ...
    fd = open(fd, mode="w+", encoding="latin-1")
    try:
...
def write_cif_image(blockname, atoms, fd, *, wrap, labels, loop_keys, moments=None):
    # writes the cif entry (of which there can be more than one to file
    fd.write(blockname)
    # writes chemical formula to file
    fd.write(chemical_formula_header(atoms))

    # writes cell parameters and space group info
    ...
    # write magnetic moments to file if exist
    if moments is not None:
        atom_moments = moments
    else:
        try:
            atom_moments = atoms.get_magnetic_moments()
        except:
            print(
                "Error getting final magnetic moments so using the initial magnetic moments"
            )
            atom_moments = atoms.get_initial_magnetic_moments()

    magnetic_headers = {
        "_atom_site_moment.label": autolabel(atoms.symbols),
        "_atom_site_moment.crystalaxis_x": [
            str(round(mom, 2)) if abs(mom) > 1 else 0 for mom in atom_moments
        ],
        "_atom_site_moment.crystalaxis_y": ["0.0"] * len(atoms.numbers),
        "_atom_site_moment.crystalaxis_z": ["0.0"] * len(atoms.numbers),
        "_atom_site_moment.symmform": ["mx,my,mz"] * len(atoms.numbers),
    }

    magnetic_loop = CIFLoop()
    formats = ["{}", "{}", "{}", "{}", "{}"]
    for i, header in enumerate(magnetic_headers.keys()):
        magnetic_loop.add(header, magnetic_headers[header], formats[i])

    fd.write(magnetic_loop.tostring())
