...
@pytest.mark.xfail
def test_run_gulp(STO_atoms, gulp_keywords, gulp_options, gulp_shells, gulp_library, gulp_conditions, expected_output,
                  monkeypatch):
    """
...
def test_run_gulp_exception(STO_atoms, gulp_keywords, gulp_options, gulp_shells, gulp_library, gulp_conditions,
    ...
    with pytest.raises(RuntimeError):
        chemdash.gulp_calc.run_gulp(STO_atoms, "gulp", "", gulp_keywords, gulp_options, gulp_shells, gulp_library,
                                    gulp_conditions)
...
def test_create_input_file_from_restart_file(test_res_file, unit_cell, gulp_keywords, gulp_options, gulp_library,
                                             expected_output, monkeypatch):
    """
...
def test_determine_vacancy_positions(STOX_structure, expected_output):
    ...
    assert all([np.allclose(x, y) for x, y in
                zip(chemdash.gulp_calc.determine_vacancy_positions(STOX_structure), expected_output)])
