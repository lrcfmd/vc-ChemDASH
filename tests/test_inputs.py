...
@pytest.mark.parametrize("keyword, value, errors, list_output, expected_output", [
    ...
    ("temp", "['4', '4', '4']", [], (),
     ("['4', '4', '4']", ['"temp" is [\'4\', \'4\', \'4\'], but should be a positive number.'])),
...
@pytest.mark.parametrize("keyword, value, errors, expected_output", [
    ...
    ("temp", "['4', '4', '4']", [],
     ("['4', '4', '4']", ['"temp" is [\'4\', \'4\', \'4\'], but all values should be positive floats.'])),
    ("temp", ['1', '1', '1'], [], ([1.0, 1.0, 1.0], [])),
    ("temp", ['4', '4', '4'], [], ([4.0, 4.0, 4.0], [])),
    ("temp", ['3.5', '3.5', '3.5'], [], ([3.5, 3.5, 3.5], [])),
    ("temp", ['4', '-4', '4'], [],
     ([4.0, -4.0, 4.0], ['"temp" is [4.0, -4.0, 4.0], but all values should be positive floats.'])),
    ("temp", ['1', '-2', '3'], [],
     ([1.0, -2.0, 3.0], ['"temp" is [1.0, -2.0, 3.0], but all values should be positive floats.'])),
])
...
@pytest.mark.parametrize("keyword, value, errors, list_output, expected_output", [
    ...
    ("grid_points", "['4', '4', '4']", [], (),
     ("['4', '4', '4']", ['"grid_points" is [\'4\', \'4\', \'4\'], but should be a positive integer.'])),
...
@pytest.mark.parametrize("keyword, value, errors, expected_output", [
    ...
    ("grid_points", "['4', '4', '4']", [],
     ("['4', '4', '4']", ['"grid_points" is [\'4\', \'4\', \'4\'], but all values should be positive integers.'])),
    ("grid_points", ['1', '1', '1'], [], ([1, 1, 1], [])),
    ("grid_points", ['4', '4', '4'], [], ([4, 4, 4], [])),
    ("grid_points", ['3.5', '3.5', '3.5'], [], (["3.5", "3.5", "3.5"], [
        '"grid_points" is [\'3.5\', \'3.5\', \'3.5\'], but all values should be positive integers.'])),
    ("grid_points", ['4', '-4', '4'], [],
     ([4, -4, 4], ['"grid_points" is [4, -4, 4], but all values should be positive integers.'])),
    ("grid_points", ['1', '-2', '3'], [],
     ([1, -2, 3], ['"grid_points" is [1, -2, 3], but all values should be positive integers.'])),
])
...
@pytest.mark.parametrize("keyword, value, errors, list_output, expected_output", [
    ...
    ("grid_points", "['4', '4', '4']", [], (),
     ("['4', '4', '4']", ['"grid_points" is [\'4\', \'4\', \'4\'], but should be an even integer.'])),
    ("grid_points", ['4', '4', '4'], [], ([4, 4, 4], []), ([4, 4, 4], [])),
])
...
@pytest.mark.parametrize("keyword, value, errors, expected_output", [
    ...
    ("grid_points", "['4', '4', '4']", [],
     ("['4', '4', '4']", ['"grid_points" is [\'4\', \'4\', \'4\'], but all values should be even integers.'])),
    ("grid_points", ['1', '1', '1'], [],
     ([1, 1, 1], ['"grid_points" is [1, 1, 1], but all values should be even integers.'])),
    ("grid_points", ['4', '4', '4'], [], ([4, 4, 4], [])),
    ("grid_points", ['3.5', '3.5', '3.5'], [], (
    ["3.5", "3.5", "3.5"], ['"grid_points" is [\'3.5\', \'3.5\', \'3.5\'], but all values should be even integers.'])),
    ("grid_points", ['4', '-4', '4'], [], ([4, -4, 4], [])),
    ("grid_points", ['1', '-2', '3'], [],
     ([1, -2, 3], ['"grid_points" is [1, -2, 3], but all values should be even integers.'])),
])
...
@pytest.mark.parametrize("keyword, value, errors, allowed_ints, expected_output", [
    ...
    ("seed_bits", "4", [], [32, 64],
     (4, ['"seed_bits" is 4, but should be one of the supported values: 32, 64'], False)),
    ("seed_bits", "64.0", [], [32, 64],
     ("64.0", ['"seed_bits" is 64.0, but should be one of the supported values: 32, 64'], False)),
    ("seed_bits", "3.5", [], [32, 64],
     ("3.5", ['"seed_bits" is 3.5, but should be one of the supported values: 32, 64'], False)),
    ("seed_bits", "foobar", [], [32, 64],
     ("foobar", ['"seed_bits" is foobar, but should be one of the supported values: 32, 64'], False)),
])
...
@pytest.mark.parametrize("keyword, value, errors, bits, expected_output", [
    ...
    ("random_seed", "4294967296", [], 32,
     (4294967296, ['"random_seed" is 4294967296, but should be an unsigned 32-bit integer.'])),
    ("random_seed", "4294967296", [], 64, (4294967296, [])),
    ("random_seed", "foobar", [], 64,
     ("foobar", ['"random_seed" is foobar, but should be an unsigned 64-bit integer.'])),
])
...
@pytest.mark.parametrize("keyword, value, errors, allowed_strings, expected_output", [
    ...
    ("verbosity", "foobar", [], ["terse", "verbose"],
     ("foobar", ['"verbosity" is foobar, but should only be one of the supported values: terse, verbose'], False)),
])
...
# ===========================================================================================================================================================
@pytest.mark.parametrize("keyword, value, errors, allowed_strings, expected_output", [
    ("old_swap_groups", ["cations", "atoms", "all"], [], ["cations", "anions", "atoms", "all"],
     (["cations", "atoms", "all"], [], True)),
    ("old_swap_groups", ["cations", "atoms", "foobar"], [], ["cations", "anions", "atoms", "all"], (
    ...
    False)),
    ("old_swap_groups", ["foo", "bar"], [], ["cations", "anions", "atoms", "all"], (["foo", "bar"], [
...
@pytest.mark.parametrize("keyword, value, errors, expected_output", [
    ...
    ("swap_groups", ["cations", "atoms", "atoms", "all"], [], (["cations", "atoms", "atoms", "all"], [
        '"swap_groups" contains atoms more than once, but each item in this list should only appear once.'], False)),
    ("swap_groups", ["cations", "cations", "all", "all"], [], (["cations", "cations", "all", "all"], [
        '"swap_groups" contains cations more than once, but each item in this list should only appear once.',
        '"swap_groups" contains all more than once, but each item in this list should only appear once.'], False)),
])
...
@pytest.mark.parametrize("swap_groups, errors, main_swap_groups, expected_output", [
    ([["cations"], ["anions"], ["atoms"], ["all"]], [], ["cations", "anions", "atoms", "all"],
     ([["cations"], ["anions"], ["atoms"], ["all"]], [], True)),
    ([["Sr-X"], ["O-Ti"]], [], ["cations", "anions", "atoms", "all"], ([["Sr-X"], ["O-Ti"]], [], True)),
    ([["Sr:X"], ["O:Ti"]], [], ["cations", "anions", "atoms", "all"], ([["Sr:X"], ["O:Ti"]], [
        ...
        '"swap_groups" contains "O:Ti", which is not a valid swap group. The main swap groups are "cations", "anions", "atoms", "all", and custom swap groups should be in the format "[Chemical Symbol]-[Chemical Symbol]-[Chemical Symbol] . . .", with each chemical symbol appearing only once.'],
                                                                       False)),
    ([["Sr-"], ["X-Sr-X"], ["O:T"]], [], ["cations", "anions", "atoms", "all"], ([["Sr-"], ["X-Sr-X"], ["O:T"]], [
...
@pytest.mark.parametrize("test_list, expected_output", [
    ...
    ("", (["", "", ""], [])),
    ("2,4,6,8", (
    ["2", "4", "6", "8"], ['"grid_points" should be a list of up to three values, but 4 values have been supplied.'])),
])
